...
\author{ECS 261}
\date{Due: Friday, January 23, 2026}
...
\item Spec 7: True
\item Spec 8: False
\item Spec 9: For $y = \texttt{abs}(x)$, either $y = x$ or $y = -x$; AND $y \ge 0$.
\end{itemize}

Your graph should contain $9$ nodes, labeled $1$ through $9$, corresponding to the $9$ specifications.
...
\textbf{Addendum:}
To make your graph more visually simple, you may omit an arrow $i \to k$ if you already have arrows $i \to j$ and $j \to k$.
...
constants.
For example, specification 1 from Problem 1 can be asserted in Z3 as follows:
\begin{verbatim}
def test_abs_z3_1():
    x = z3.Int('x')
...
\item
What is one advantage and one drawback on the effectiveness of this approach to verifying specifications, compared to the testing approach in Problem 1?
\end{enumerate}
...
Assume that any source-to-source compiler $C$ should preserve the semantics of the Python code on any input in the following sense: any source program $P$ on any input $x$ should have the same observable behavior as the compiled program $C(P)$ on input $x$, and should produce the same output.
...
\begin{enumerate}[(a)]
\item
Write a solver for this game using Z3. Your solver should be provided as two functions: the first function \texttt{q5a(a, b, c, d)} should, when given four Python integers $a, b, c, d$, return a solution as Python integers $x, y$, if there is at least one solution, or (\texttt{None}, \texttt{None}) if there is no solution.
...
\texttt{q5b(a, b, c, d, x, y)} should return a Python string ``multiple'', ``unique'', or ``none'' depending on whether multiple solutions exist - given the four Python integers and the output $(x, y)$ from part (a) (x, y will be (\texttt{None}, \texttt{None} if there was no solution from part (a)).
Update your \texttt{q5\_run} interactive version to also show the output of \texttt{q5b}.
\end{enumerate}
